...
def items_appear_in_both_compartment(rucksack_elf1, rucksack_elf2, rucksack_elf3):
    first = set(rucksack_elf1)
    second = set(rucksack_elf2)
    third = set(rucksack_elf3)
    items_both = ''
    for i in first:
...
if __name__ == '__main__':
    prioriry = "_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    sum_of_priority = 0
    for i in range(0, len(arr_lst), 3):
        sum_of_priority += prioriry.index(
            items_appear_in_both_compartment(arr_lst[i], arr_lst[i+1], arr_lst[i+2]))
    print("Sum of the priorities is", sum_of_priority)
